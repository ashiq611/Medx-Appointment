class DoctorRepo {
  ...
  getTotalDoctors = async (client: any) => {
    try {
      const query = {
        text: "SELECT COUNT(*) FROM public.doctor",
      };
      const responseData = await client.query(query);
  ...
  getTotalDayWiseAppointment = async (client: any) => {
    try {
      const query = {
        text: `
            SELECT
  hb.branchname AS hospital_branch_name,
  TO_CHAR(a.appointmentdate, 'Day') AS appointment_day,
  COUNT(*) AS appointment_count
FROM
  public.appointment a
JOIN
  public.hospitalbranch hb ON a.hospitalbranchid = hb.hospitalbranchid
WHERE
  a.appointmentdate >= CURRENT_DATE - INTERVAL '7 days'
GROUP BY
  hb.branchname, TO_CHAR(a.appointmentdate, 'Day')
ORDER BY
  appointment_day DESC, hb.branchname;
            `,
      };
      const responseData = await client.query(query);
      return responseData.rows;
  ...
  getTotalBranchWiseDoctor = async (client: any) => {
    try {
      const query = {
        text: `
            SELECT
  hb.branchname AS hospital_branch_name,
  COUNT(d.doctorid) AS total_doctors
FROM
  public.hospitalbranch hb
JOIN
  public.doctor d ON hb.hospitalbranchid = d.hospitalbranchid
GROUP BY
  hb.branchname
ORDER BY
  hb.branchname;
